...
const FR: Dict = {
  ...
  'settings.vaultHeading': "Réglages du vault (.obsidian)",
  'settings.vaultDesc': "Transférez vos réglages Obsidian (apparence, raccourcis, plugins installés et LEURS réglages) entre appareils, via le dossier de travail Drive. Ce plugin est toujours exclu : votre jeton reste local. La disposition des panneaux est exclue aussi (propre à chaque appareil). Attention : les réglages des autres plugins peuvent contenir des clés d'API.",
...
const EN: Dict = {
  ...
  'settings.vaultHeading': "Vault settings (.obsidian)",
  'settings.vaultDesc': "Transfer your Obsidian settings (appearance, hotkeys, installed plugins and THEIR settings) between devices, through the Drive working folder. This plugin is always excluded: your token stays local. Pane layout is excluded too (device-specific). Warning: other plugins' settings may contain API keys.",
